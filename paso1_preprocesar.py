...
def extract_dicom_images_patient(src_dir):
    target_dir = configuracion.NDSB3_EXTRACTED_IMAGE_DIR
    patient_id = src_dir
    patient_dir = target_dir + patient_id + "/"
    dir_path = configuracion.NDSB3_RAW_SRC_DIR + src_dir + "/"
    if os.path.isdir(patient_dir):
        print("Dir: " + src_dir + ", ya existe")
        return
    else:
        print("Dir: ", src_dir)
    slices = load_patient(dir_path)
    print(len(slices), "\t", slices[0].SliceThickness, "\t", slices[0].PixelSpacing)
    print("Orientacion: ", slices[0].ImageOrientationPatient)
    ...
    print("Invertir orden: ", invert_order, " - ", slices[1].ImagePositionPatient[2], ",", slices[0].ImagePositionPatient[2])

    pixel_spacing = slices[0].PixelSpacing
    pixel_spacing.append(slices[0].SliceThickness)
    image = herr.reescalar_imagenes_paciente(image, pixel_spacing, configuracion.TAM_VOXEL_MM)
    if not invert_order:
    ...
    for i in range(image.shape[0]):
        ...
        img, mask = herr.segmentar_pulmones(org_img.copy())
        org_img = herr.normalizar_hu(org_img)
...
def extract_dicom_images(clean_targetdir_first=False, only_patient_id=None):
    print("estrayendo imagenes")

    target_dir = configuracion.NDSB3_EXTRACTED_IMAGE_DIR
    if clean_targetdir_first and only_patient_id is not None:
        print("Limpiando directorio objetivo")
        for file_path in glob.glob(target_dir + "*.*"):
            os.remove(file_path)

    if only_patient_id is None:
        dirs = os.listdir(configuracion.NDSB3_RAW_SRC_DIR)
        if True:
            pool = Pool(configuracion.N_HILOS)
